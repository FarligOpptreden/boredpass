...
export default class CrudController {
  constructor(resourceContext, service, args) {
    ...
    this.searchFields =
      args && args.searchFields ? args.searchFields : ["name"];
  ...
  buildFilter(search) {
    if (!this.searchFields && !search) return null;
  ...
  get controllers() {
    return (
      Controller.create(this.resourceContext)
        // Page service data
        .handle(
          { route: "/page/:pageNo", method: "get", produces: "json" },
          (req, res) => {
            this.service.page(
              {
        ...
        // Create service data
        .handle(
          { route: "/", method: "post", produces: "json", consumes: "json" },
          (req, res) => {
            this.service.create(
              {
                data: req.body
        ...
        .handle(
          { route: "/:uuid", method: "get", produces: "json" },
          (req, res) => {
